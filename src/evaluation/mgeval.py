...
def __analyze(feature, bpm: int, length_in_bars: int = None, normalize: bool = False):
    ...
    return {
        ...
        'pitch_count_per_bar': 0.0, # metrics.bar_used_pitch(feature, 1, length_in_bars), # CHECK calculation in core seems to return wrong results
        'pitch_class_histogram': metrics.total_pitch_class_histogram(feature),
        'pitch_class_histogram_per_bar': 0.0, # metrics.bar_pitch_class_histogram(feature, 0, bpm, length_in_bars), # CHECK only works when length in bars is given, should have reliable calculation here
        ...
        'note_count_per_bar': 0.0, # metrics.bar_used_note(feature, 1, length_in_bars), # CHECK calculation in core seems to return wrong results
